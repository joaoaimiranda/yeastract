...
export default function hypergeometric(N, R, n, r) {
    ...
    const popSize = Math.floor(N);

    // Get the number of successes in the population
    const successesAvailable = Math.floor(R);

    // Get the number of items sampled
    const sampled = Math.floor(n);

    // Get the number of successes observed
    const successesObserved = Math.floor(r);
    ...
    Prob = Math.round((1 - Prob) * 100000) / 100000;
    return Prob;
}
